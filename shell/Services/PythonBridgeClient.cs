...
public sealed class PythonBridgeClient : IAsyncDisposable, IDisposable
{
    ...
    public async Task<JsonElement> InvokeAsync(string action, object? payload = null, CancellationToken cancellationToken = default)
    {
        if (_disposed || _shutdownCts.IsCancellationRequested)
            throw new ObjectDisposedException(nameof(PythonBridgeClient), "Project Factory 正在退出。");

        if (!File.Exists(_pythonExe))
            throw new FileNotFoundException("Project Factory 私有 Python Core 运行时缺失。请运行安装器 Repair/重新安装。", _pythonExe);
        if (!File.Exists(_bridgeScript))
            throw new FileNotFoundException("Project Factory backend bridge 缺失。", _bridgeScript);
        ...
        try
        {
            ...
            using (token.Register(() => tcs.TrySetCanceled()))
            {
                ...
                catch (OperationCanceledException) when (_shutdownCts.IsCancellationRequested)
                {
                    ...
                    throw new OperationCanceledException("Project Factory 正在退出，后台进程已终止。");
                }
    ...
    private async Task EnsureResidentProcessAsync(CancellationToken token)
    {
        ...
        try
        {
            ...
            Log("RESIDENT START");
            if (!process.Start())
                throw new InvalidOperationException("无法启动 Project Factory backend。");
